...
namespace NexusDogsGo.Gameplay.World
{
    public readonly struct GeoCoordinate
    {
        ...
        {
            Latitude = Math.Max(-90d, Math.Min(90d, latitude));
    ...
    public static class GeoMath
    {
        ...
        public static GeoCoordinate OffsetMeters(in GeoCoordinate origin, double northMeters, double eastMeters)
        {
            ...
            var cos = Math.Cos(DegreesToRadians(origin.Latitude));
            var safeCos = Math.Abs(cos) < 0.000001d ? 0.000001d : cos;
            var dLon = eastMeters / (EarthRadiusMeters * safeCos);
